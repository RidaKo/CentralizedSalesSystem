    ...
    public class OrderService : IOrderService
    {
            ...
            long? filterByTableId = null)
        {
            var query = _db.Orders
                ...
                .Include(o => o.Items)
                    .ThenInclude(oi => oi.ItemVariationOption)
                        .ThenInclude(vo => vo.ItemVariation)
                .Include(o => o.Items)
                ...
                .Include(o => o.Items)
                    .ThenInclude(i => i.Item)
                        .ThenInclude(it => it.Variations)
                            .ThenInclude(v => v.Options)
                .Include(o => o.Items)
                    .ThenInclude(i => i.ItemVariationOption)
                        .ThenInclude(vo => vo.ItemVariation)
                .Include(o => o.Items)
        ...
        {
            decimal ResolveUnitPrice(OrderItem orderItem) =>
                orderItem.Item.Price + (orderItem.ItemVariationOption?.PriceAdjustment ?? 0m);

            dto.Subtotal = order.Items.Sum(i => i.Quantity * ResolveUnitPrice(i));
            ...
            decimal itemLevelDiscount = order.Items.Sum(i =>
            {
                ...
                var unitPrice = ResolveUnitPrice(i);
                return (i.Discount.rate / 100m) * i.Quantity * unitPrice;
            });
            ...
            dto.TaxTotal = order.Items.Sum(i =>
            {
                ...
                decimal itemDiscount = i.Discount != null && i.Discount.AppliesTo == DiscountAppliesTo.Product
                    ? (i.Discount.rate / 100m) * i.Quantity * unitPrice
                    : 0m;

                decimal taxableAmount = i.Quantity * unitPrice - itemDiscount;
            ...
            dto.ServiceChargeTotal = order.Items.Sum(i =>
            {
                ...
                decimal baseAmount = i.Quantity * unitPrice - itemDiscount;
